...
class LoanApplication(BaseLoanApplication):
	...
	def on_update(self):
		...
		if self.status == "Approved" and self.get_db_value("status") != "Approved":
			...
			from core.rlending.api.loan import disburse_loan
	...
	def validate_kyc(self):
		...
		if self.applicant_type == "Customer":
			...
			if not lead and customer_mobile:
				lead = frappe.db.get_value("CRM Lead", {"mobile_no": customer_mobile}, "name")
				
			if lead:
				kyc_status = frappe.db.get_value("CRM Lead", lead, "kyc_status")
				if kyc_status != "Verified":
					frappe.throw(_("KYC Verification is required for withdrawable loans. Current status: {0}").format(kyc_status))
