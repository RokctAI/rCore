...
def _send_via_control_relay(**kwargs):
    ...
    try:
        ...
        api_url = f"{scheme}://{control_plane_url}/api/method/rokct.control.api.relay_email"
        ...
        if response_json.get("status") != "success":
            # If the relay fails, log it and raise an exception to notify the caller.
            frappe.log_error(f"Failed to relay email via control panel. Response: {response_json}", "Email Relay Error")
            raise frappe.ValidationError("Failed to send email via the central mailer.")

    except Exception as e:
        # Catch any exception and re-raise it to ensure the caller knows the email failed.
...
def send_tenant_email(**kwargs):
    ...
    """
    try:
        # 1. Check if a default outgoing email account is configured and enabled on this site.
        default_account = frappe.db.get_value("Email Account", {"default_outgoing": 1, "enable_outgoing": 1})
        if default_account:
