    ...
    def decorator(fn):
        ...
        def wrapper(*args, **kwargs):
            ...
            if not subscription:
                ...
                if subscription:
                    # Use the cache duration from the subscription details, or default to 24 hours
            ...
            if not subscription:
                frappe.throw("Could not retrieve subscription details.", frappe.PermissionError)
            ...
            if feature_module not in subscription.get("modules", []):
                frappe.throw(f"Your plan does not include the '{feature_module}' feature.", frappe.PermissionError)
