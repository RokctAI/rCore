  ...
  frappe.checkout.razorpay = class RazorpayCheckout {
    ...
    get_key() {
      return new Promise((resolve) => {
        frappe
          .call(
            "rcore.gateways.doctype.razorpay_settings.razorpay_settings.get_api_key"
          )
          .then((res) => {
    ...
    make_order() {
      return new Promise((resolve) => {
        frappe
          .call(
            "rcore.gateways.doctype.razorpay_settings.razorpay_settings.get_order",
            {
    ...
    order_success(response) {
      frappe.call(
        "rcore.gateways.doctype.razorpay_settings.razorpay_settings.order_payment_success",
        {
    ...
    order_fail(response) {
      frappe.call(
        "rcore.gateways.doctype.razorpay_settings.razorpay_settings.order_payment_failure",
        {
          integration_request: this.order.integration_request,
          params: response,
        }
