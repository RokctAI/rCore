...
from rcore.gateways.doctype.mpesa_settings.mpesa_connector import MpesaConnector
from rcore.gateways.doctype.mpesa_settings.mpesa_custom_fields import (
	create_custom_pos_fields,
)
from rcore.gateways.utils import erp_module_available, erpnext_app_import_guard
...
class MpesaSettings(Document):
	...
	def on_update(self):
		from rcore.gateways.utils import create_payment_gateway
...
def generate_stk_push(**kwargs):
	...
	try:
		callback_url = (
			...
			+ "/api/method/rcore.gateways.doctype.mpesa_settings.mpesa_settings.verify_transaction"
		)
...
def get_account_balance(request_payload):
	...
	try:
		...
		callback_url = (
			get_request_site_address(True)
			+ "/api/method/rcore.gateways.doctype.mpesa_settings.mpesa_settings.process_balance_info"
		)
...
def create_mode_of_payment(gateway, payment_type="General"):
	with erpnext_app_import_guard():
		# ROKCT: cross-module import into the composed erp module, resolved from
		# __name__ — doctype-tree files must not carry the composer's app-name
		# token for cross-module paths (crm-port precedent). Upstream imported
		# from the separate erpnext app; the guard still catches ImportError.
		from importlib import import_module

		get_default_company = import_module(
			__name__.split(".gateways.doctype.", 1)[0] + ".erp.erp_init"
		).get_default_company
