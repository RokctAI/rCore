...
def erp_module_available():
	"""True when the composed `erp` module (pay SDK's ERPNext port) is part
	of this app. Replaces upstream's check for the separate `erpnext` app
	(ROKCT self-containment remap)."""
	try:
		return find_spec("rcore.erp") is not None
...
def erpnext_app_import_guard():
	msg = _(
		"The erp module (pay SDK's ERPNext port) is not part of this app. "
		"Compose the erp module alongside gateways to enable this feature."
	)
	try:
		yield
	except ImportError:
