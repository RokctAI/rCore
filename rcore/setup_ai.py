    ...
    if not os.path.exists(model_dir):
        try:
            ...
            try:
                ...
                subprocess.run(["sudo", "chown", "-R", f"{os.environ.get('USER')}:{os.environ.get('USER')}", model_dir], check=True)
                print(f"✅ Created {model_dir}")
            except subprocess.CalledProcessError:
                print(f"⚠️ Could not create {model_dir} (Permission Denied). Falling back to ~/rokct_models.")
    ...
    try:
        ...
        for task in tasks:
            ...
            if backend == "local":
                ...
                try:
                    snapshot_download(repo_id=task["repo"], local_dir=local_path, local_dir_use_symlinks=False)
                ...
                if os.path.exists(local_path):
                    print(f"🧹 {task['name']}: Backend is {backend.upper()}. Cleaning up local model...")
                    try:
