...
def generate_release_notes(
    repo_url: str, commit_log: str, version_name: str = "vNext"
) -> dict:
    """
    ...
    log_info(
        f"Generating release notes for repo_url: {repo_url}, version_name: {version_name}"
    )

    if frappe.session.user == "Guest":
        log_error(
            "Authentication required: Guest user attempted to generate release notes."
        )
        frappe.throw(
