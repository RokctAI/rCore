...
class JulesClient:
    ...
    def delete_session(self, api_key, session_id):
        ...
        url = f"{self.BASE_URL}/sessions/{session_id}"
        try:
            response = requests.delete(url, headers=self._get_headers(api_key), timeout=10)
            response.raise_for_status()
    ...
    def approve_plan(self, api_key, session_id):
        ...
        try:
            response = requests.post(url, headers=self._get_headers(api_key), json={}, timeout=10)
            response.raise_for_status()
    ...
    def send_message(self, api_key, session_id, message):
        ...
        try:
            response = requests.post(url, headers=self._get_headers(api_key), json=payload, timeout=30)
            response.raise_for_status()
