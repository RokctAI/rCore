...
def _should_route_extensions(worker_type):
    ...
    """
    try:
        config_path = frappe.get_site_path(
        ...
        if os.path.exists(config_path):
            with open(config_path, "r") as f:
                ...
                backend = config.get(
                    f"{worker_type}_backend",
                    "jina" if worker_type == "vision" else "local",
                )
...
def _call_jina_vision(data):
    ...
    if not file_url and data.get("file_path"):
        return {
            "status": "error",
            "message": "Jina Backend requires 'file_url'. Local file paths are not supported yet.",
        }
...
def ask_brain(prompt, system=None, tools=None):
    ...
    try:
        url = (
            "http://127.0.0.1:8642/v1/v1/chat/completions"
            if os.environ.get("ROK_API_URL")
            else "http://127.0.0.1:8642/v1/chat/completions"
        )
        headers = {
