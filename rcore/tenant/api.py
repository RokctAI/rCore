...
def send_error_to_control(doc):
    ...
    try:
        ...
        frappe.log_error(frappe.get_traceback(),
                         "Failed to forward error to control panel")
...
def record_token_usage(tokens_used: int, model_name: str = "flash"):
    ...
    try:
        ...
        # Total aggregation
        user_doc.daily_token_usage = (
            user_doc.daily_token_usage or 0) + tokens_used
        user_doc.monthly_token_usage = (
            user_doc.monthly_token_usage or 0) + tokens_used
        ...
        if "pro" in model_name.lower() and "flash" not in model_name.lower():
            user_doc.daily_pro_usage = (
                user_doc.daily_pro_usage or 0) + tokens_used
        else:
            user_doc.daily_flash_usage = (
                user_doc.daily_flash_usage or 0) + tokens_used
...
def get_token_usage():
    ...
    try:
        ...
        monthly_limit = subscription_details.get(
            "monthly_token_limit", 0)  # Base/Flash Limit
        monthly_paid_limit = subscription_details.get(
            "monthly_paid_token_limit", 0)  # Pro Limit
        is_per_seat_plan = subscription_details.get("is_per_seat_plan", 0)
        ...
        if is_per_seat_plan:
            ...
            if frappe.db.exists("User", current_user):
                user_val = frappe.db.get_value(
                    "User", current_user, "ai_seat_assigned")
                if not user_val:
                    # Not assigned, try to assign
                    user_quantity = subscription_details.get(
                        "user_quantity", 0) or 0
                    base_user_count = subscription_details.get(
                        "base_user_count", 0) or 0
                    ...
                    if used_seats < limit:
                        # Auto-assign
                        frappe.db.set_value(
                            "User", current_user, "ai_seat_assigned", 1)
                        frappe.db.commit()
                    else:
        ...
        daily_pro_remaining = daily_pro_limit - \
            daily_pro_usage if daily_pro_limit > 0 else -1
        daily_flash_remaining = daily_flash_limit - \
            daily_flash_usage if daily_flash_limit > 0 else -1
...
def initial_setup(
    ...
    try:
        ...
        if not frappe.db.exists("Warehouse Type", "Transit"):
            frappe.get_doc({"doctype": "Warehouse Type",
                            "name": "Transit"}).insert(ignore_permissions=True)
...
def resend_verification_email(email: str):
    ...
    try:
        ...
        verification_url = get_url(
            f"/api/method/rcore.tenant.api.verify_my_email?token={token}")
        email_context = {
        ...
        frappe.throw(
            f"An error occurred while resending the verification email: {e}")
...
def create_temporary_support_user(
    ...
    try:
        ...
        frappe.log_error(
            frappe.get_traceback(),
            "Temporary Support User Creation Failed")
...
def disable_temporary_support_user(support_user_email):
    ...
    if not support_user_email:
        frappe.throw(
            "Support User Email is required.",
            title="Missing Information")
    try:
    ...
    try:
        ...
        return {
            "status": "success",
            "message": f"Support user {support_user_email} has been disabled."}

    except Exception as e:
        frappe.db.rollback()
        frappe.log_error(
            frappe.get_traceback(),
            f"Failed to disable support user {support_user_email}")
        frappe.throw(
            f"An error occurred while disabling the support user: {e}")
...
def create_sales_invoice(
    ...
    # --- Input Validation ---
    if not isinstance(invoice_data, dict) or not invoice_data.get(
            "customer") or not invoice_data.get("items"):
        frappe.throw(
    ...
    try:
            ...
            auto_repeat = frappe.get_doc(
                {
                    ...
                    "frequency": frequency,
                    "end_date": end_date}).insert(
                ignore_permissions=False)
            auto_repeat.submit()
            response_data["auto_repeat_name"] = auto_repeat.name
            response_data["message"] = f"Sales Invoice {
                invoice_doc.name} created and scheduled for recurring generation."
        else:
            response_data["message"] = f"Sales Invoice {
                invoice_doc.name} created successfully."
        ...
        frappe.throw(
            f"An error occurred while creating the Sales Invoice: {e}")
...
def log_frontend_error(error_message, context=None):
    ...
    if not error_message or not isinstance(
            error_message, str) or not error_message.strip():
        return {
            "status": "error",
    ...
    try:
        ...
        if context:
            try:
                ...
                if isinstance(context_data, dict):
                    ...
                    reference_doctype = context_data.get(
        ...
        return {
            "status": "error",
            "message": "Failed to log error to backend."}
...
def get_subscription_details():
    ...
    try:
        ...
        if details and isinstance(details, dict):
            cache_duration_seconds = details.get(
                "subscription_cache_duration", 86400)
            frappe.cache().set_value(
                "subscription_details",
                details,
                expires_in_sec=cache_duration_seconds)
        ...
        frappe.log_error(
            frappe.get_traceback(),
            "Subscription Details Proxy Failed")
...
def set_platform_secret(secret: str):
    ...
    if "System Manager" not in frappe.get_roles():
        frappe.throw(
            "Only System Managers can set the Platform Secret.",
            frappe.PermissionError)
...
def get_welcome_email_details():
    ...
    try:
        ...
        frappe.log_error(
            frappe.get_traceback(),
            "Failed to get welcome email details")
        frappe.throw(
            f"An error occurred while getting welcome email details: {e}")
...
def update_verification_token(email, token):
    ...
    try:
        ...
        frappe.log_error(
            frappe.get_traceback(),
            "Update Verification Token Failed")
        frappe.throw(
            f"An error occurred while updating the verification token: {e}")
...
def update_fiscal_year_if_default(start_date):
    ...
    try:
        ...
        current_fy_doc = frappe.get_doc(
            "Fiscal Year", current_fiscal_year_name)
        print(
            f"DEBUG: Current fiscal year is '{current_fiscal_year_name}' with start date {
                current_fy_doc.year_start_date}")
...
def complete_onboarding():
    ...
    try:
        ...
        if not default_company_link:
            frappe.throw(
                "No default company found for the current user.",
                title="Not Found")
        ...
        frappe.throw(
            f"An error occurred while marking onboarding as complete: {e}")
