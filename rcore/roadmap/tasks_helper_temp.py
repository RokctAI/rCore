...
def _construct_contextual_prompt(roadmap, feature, mode="Building"):
    ...
    template = next((p for p in prompts if p.type == feature.type and p.mode == mode), None)
    ...
    # The template is just the "Persona/System" part. We need to append the actual task.
