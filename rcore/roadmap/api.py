...
import hashlib
import re
from rcore.roadmap.tasks import _get_api_key, _create_jules_session
...
def update_task_status_from_pr():
    ...
    if signature_type != 'sha256':
        frappe.throw(
            "Authentication failed: Unsupported signature type.",
            frappe.PermissionError)
    ...
    feature_doc_name = frappe.db.get_value(
        "Roadmap Feature", {"jules_session_id": session_id})
    if feature_doc_name:
...
def setup_github_workflow(roadmap_name):
    ...
    if not workflow_template:
        frappe.throw(
            "GitHub Action YAML is not configured in Roadmap Settings.")
...
Do not ask for a plan. Do not write any other code. Just create the file and the pull request."""
    ...
    session_id = _create_jules_session(
        jules_api_key, repo_url, "Setup ROKCT Workflow", prompt)
