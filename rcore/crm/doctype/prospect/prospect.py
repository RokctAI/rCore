...
class Prospect(CRMNote):
	...
	if TYPE_CHECKING:
		...
		from ..prospect_lead.prospect_lead import ProspectLead
		from ..prospect_opportunity.prospect_opportunity import ProspectOpportunity
	...
	def update_exchange_rate(self):
		# ROKCT (ported from the standalone CRM Organization controller):
		# exchange-rate providers are not part of this slice — organizations
		# are valued in a single currency until a currency service composes in.
		if not self.exchange_rate:
			self.exchange_rate = 1

	# --- ROKCT platform SDK contract (ported from the standalone
	# CRM Organization controller): api.crm.doc.* reads these statics.
