...
frappe.ui.form.on("Opportunity", {
	...
	party_name: function (frm) {
		...
		if (frm.doc.opportunity_from == "Customer") {
			...
			erpnext.utils.map_current_doc({
				method: "rcore.crm.doctype.lead.lead.make_opportunity",
	...
	currency: function (frm) {
		...
		if (company_currency != frm.doc.currency) {
			frappe.call({
				method: "rcore.erp.setup.utils.get_exchange_rate",
				args: {
	...
	make_supplier_quotation: function (frm) {
		frappe.model.open_mapped_doc({
			method: "rcore.crm.doctype.opportunity.opportunity.make_supplier_quotation",
			frm: frm,
		});
	},
	...
	make_request_for_quotation: function (frm) {
		frappe.model.open_mapped_doc({
			method: "rcore.crm.doctype.opportunity.opportunity.make_request_for_quotation",
			frm: frm,
		});
	},
...
erpnext.crm.Opportunity = class Opportunity extends frappe.ui.form.Controller {
	...
	setup_queries() {
		...
		this.frm.set_query("item_code", "items", function () {
			return {
				query: "rcore.erp.controllers.queries.item_query",
		...
		this.frm.set_query("uom", "items", function (doc, cdt, cdn) {
			...
			return {
				query: "rcore.erp.controllers.queries.get_item_uom_query",
				filters: {
	...
	create_quotation() {
		frappe.model.open_mapped_doc({
			method: "rcore.crm.doctype.opportunity.opportunity.make_quotation",
			frm: this.frm,
		});
	}
	...
	make_customer() {
		frappe.model.open_mapped_doc({
			method: "rcore.crm.doctype.opportunity.opportunity.make_customer",
			frm: this.frm,
		});
	}
...
cur_frm.cscript.item_code = function (doc, cdt, cdn) {
	...
	if (d.item_code) {
		return frappe.call({
			method: "rcore.crm.doctype.opportunity.opportunity.get_item_details",
