...
def reset_monthly_token_usage():
    ...
    for item in trackers_to_reset:
        try:
            ...
            frappe.log_error(
                f"Failed to reset token tracker: {e}", "Token Usage Job Failed"
            )
    frappe.db.commit()
...
def update_storage_usage():
    ...
    try:
        ...
        frappe.db.rollback()
        frappe.log_error(
            f"Storage usage calculation failed: {e}", "Storage Usage Job Failed"
        )
...
def _fetch_and_upsert_stimuli():
    ...
    relevant_tenders = call_control(
        "tenders.get_relevant_tenders", {"filters": json.dumps(filters)}
    )
...
def _delete_expired_stimuli():
    expired = frappe.get_all(
        "Stimulus", filters={"stimulus_end_date": ["<", nowdate()]}, fields=["name"]
    )
    for s in expired:
...
def _delete_expired_funding():
    expired = frappe.get_all(
        "Neurotrophin", filters={"deadline": ["<", nowdate()]}, fields=["name"]
    )
    for f in expired:
...
def check_invoice_payments():
    ...
    for inv in unpaid_invoices:
        try:
            ...
            frappe.log_error(
                f"Failed payment check for {inv.name}: {e}",
                "Invoice Payment Check Error",
            )
...
def pick_proactive_question():
    ...
    questions = frappe.get_all(
        "Question Bank", filters={"is_active": 1}, fields=["question", "category"]
    )
    if not questions:
        return
    ...
    frappe.log_error(
        f"Proactive Question selected: {selected.question}",
        "Proactive Question Bank Picker",
    )
...
def send_weekly_goal_reminders():
    ...
    active_goals = frappe.get_all(
        ...
        filters={"status": ["not in", ["Achieved", "Cancelled"]]},
        fields=["name", "title"],
    )
...
def archive_inactive_vault_files():
    ...
    expired_tenants = frappe.get_all(
        "User",
        filters={"enabled": 0, "temporary_user_expires_on": ["<", ninety_days_ago]},
        fields=["name", "email"],
    )
    ...
    for tenant in expired_tenants:
        ...
        for f in files:
            try:
                ...
                frappe.log_error(
                    f"Failed to archive/delete file {f.name} for {tenant.email}: {e}"
                )
    frappe.db.commit()
...
def check_protocol_99_sequences():
    ...
    active_releases = frappe.get_all(
        "Legacy Vault",
        filters={
    ...
    for release in active_releases:
        try:
            ...
            if relationship:
                frappe.db.set_value(
                    "Legacy Vault", release.name, "release_status", "Released"
                )
                frappe.log_error(
...
def tag_engram_pillars(doc, method=None):
    ...
    tags = []
    if any(
        w in text for w in ["business", "invoice", "revenue", "operations", "venture"]
    ):
        tags.append("Business")
    if any(
        w in text for w in ["career", "promotion", "milestone", "job", "profession"]
    ):
        tags.append("Career")
    if any(
        w in text for w in ["goal", "productivity", "accountability", "habit", "wins"]
    ):
...
def archive_low_score_engrams():
    ...
    if frappe.db.exists("DocType", "Engram"):
        low_score_engrams = frappe.get_all(
            "Engram",
            filters={"creation": ["<", one_year_ago], "relevance_score": ["<", 0.5]},
            fields=["name"],
        )
