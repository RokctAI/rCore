...
def reset_monthly_token_usage():
    ...
    frappe.log(
        f"Found {len(trackers_to_reset)} token trackers to reset.", "Token Usage Job"
    )
...
def update_storage_usage():
    ...
    try:
        ...
        total_size_bytes = (
            frappe.db.sql("SELECT SUM(file_size) FROM `tabFile`")[0][0] or 0
        )
        ...
        frappe.log(
            f"Successfully updated storage usage to {total_size_mb:.2f} MB.",
            "Storage Usage Job",
        )
...
def disable_expired_support_users():
    ...
    expired_users = frappe.get_all(
        "User",
        filters={"enabled": 1, "temporary_user_expires_on": ["<", now_datetime()]},
    ...
    for user_info in expired_users:
        try:
            ...
            frappe.log_error(
                frappe.get_traceback(),
                f"Failed to disable expired support user {user_info.email}",
            )
...
def _fetch_monorepo_via_ssh(repo_path, target_root):
    ...
    try:
        ...
        frappe.log_error(
            f"SSH Fetch Failed: {e.stderr.decode()}", "Monorepo SSH Sync Error"
        )
    except Exception as e:
        frappe.log_error(
            f"Unexpected error during SSH sync: {e}", "Monorepo SSH Sync Error"
        )
    finally:
        ...
        for md_file in sync_path.glob("*.md"):
            ...
            try:
                ...
                if checklist_section:
                    items = re.findall(
                        r"-\s+\[\s*\]\s*(.*?)\|\s*(\d+)", checklist_section.group(1)
                    )
                ...
                release = {
                    ...
                    "tender": {
                        ...
                        "procuringEntity": {
                            "name": institution_match.group(1).strip()
                            if institution_match
                            else "Unknown"
                        },
                        "tenderPeriod": {
                            "endDate": closing_match.group(1).strip()
                            if closing_match
                            else ""
                        },
                ...
                frappe.log_error(
                    f"Failed to process fetched tender {md_file.name}: {e}",
                    "Tender Sync Error",
                )
        ...
        for release in relevant_tenders:
            try:
                ...
                organ_of_state = _get_linked_doc_name(
                    "Organ of State", tender_data.get("procuringEntity", {}).get("name")
                )
                ...
                stimulus_doc_data = {
                    ...
                    "stimulus_end_date": _format_datetime_str(
                        get_datetime(tender_data.get("tenderPeriod", {}).get("endDate"))
                    ),
                    ...
                    "province": province,
                    "esubmission": 1
                    if "electronicSubmission" in tender_data.get("submissionMethod", [])
                    else 0,
                    "custom_workflow_json": json.dumps(
                        tender_data.get("custom_tasks", [])
                    ),
                }
                ...
                frappe.log_error(
                    f"Failed to process release {release.get('ocid')}: {e}",
...
def _get_linked_doc_name(doctype, value):
    ...
    if not fieldname:
        frappe.log_error(
            f"No field mapping found for Doctype {doctype}", "Stimulus Linking Error"
        )
        return None
    try:
...
def _delete_expired_stimuli():
    ...
    try:
        ...
        expired_stimuli = frappe.get_all(
            "Stimulus", filters={"stimulus_end_date": ["<", today]}, fields=["name"]
        )
        ...
        for stimulus in expired_stimuli:
            frappe.delete_doc(
                "Stimulus", stimulus.name, ignore_permissions=True, force=True
            )
    ...
    with tempfile.TemporaryDirectory() as sync_dir:
        ...
        for md_file in sync_path.glob("*.md"):
            ...
            try:
                ...
                frappe.log_error(
                    f"Failed to process fetched grant {md_file.name}: {e}",
                    "Funding Sync Error",
                )
...
def _process_funding_cache():
    ...
    for item in soup.select(".listing-item"):
        try:
            ...
            frappe.log_error(
                f"Failed to process funding item: {e}", "Funding Processing Failed"
            )
    ...
    frappe.delete_doc(
        "Raw Neurotrophin Cache", raw_cache[0].name, ignore_permissions=True, force=True
    )
    frappe.db.commit()
...
def _delete_expired_funding():
    ...
    try:
        today = getdate(nowdate())
        expired_funding = frappe.get_all(
            "Neurotrophin", filters={"deadline": ["<", today]}, fields=["name"]
        )
        ...
        for fund in expired_funding:
            frappe.delete_doc(
                "Neurotrophin", fund.name, ignore_permissions=True, force=True
            )
