...
class BankStatementParser:
    ...
    def _calculate_metrics(self):
        ...
        overdrafts = sum(1 for t in self.transactions if t['amount'] < 0 and 'overdraft' in t['description'].lower())
    ...
    def _extract_bank_details(self):
        ...
        # 2. Branch Code
        branch_pattern = re.compile(r'(?:Branch Code|Branch No|Sort Code)[\s:]+(\d{5,6})', re.IGNORECASE)
        branch_match = branch_pattern.search(text)
        if branch_match:
            self.bank_details['bank_branch_code'] = branch_match.group(1)

        # 3. Bank Name
        banks = ["Absa", "Standard Bank", "FNB", "Nedbank", "Capitec", "Investec", "Discovery Bank", "TymeBank"]
        for bank in banks:
        ...
        holder_pattern = re.compile(r'(?:Account Holder|Name|Customer Name)[\s:]+([A-Z ]{5,})', re.IGNORECASE)
        holder_match = holder_pattern.search(text)
        if holder_match:
             # Basic cleanup: take first line, strip
             self.bank_details['bank_account_holder'] = holder_match.group(1).split('\n')[0].strip()
