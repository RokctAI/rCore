...
)  # C:\Users\sinya\Desktop\RokctAI\rcore\rcore\tests
rcore_base = os.path.dirname(
    os.path.dirname(current_file_dir)
)  # C:\Users\sinya\Desktop\RokctAI\rcore
...
SYS_PATHS = [rcore_base, control_base]
for p in SYS_PATHS:
...
class MockDoc:
    ...
    def insert(self, ignore_permissions=False):
        print(
            f"   [Mock DB] INSERT {self.doctype} document: '{self.title or self.name}'"
        )
        return self

    def save(self, ignore_permissions=False):
        print(
            f"   [Mock DB] SAVE {self.doctype} document: '{self.name}' with vision='{self.vision}'"
        )
        return self
...
frappe_mock.flags = types.SimpleNamespace(
    in_test=True, in_migrate=False, in_install=False
)
...
def get_all(doctype, filters=None, **kwargs):
    print(
        f"   [Mock DB] GET ALL {doctype} with filters: {filters} and kwargs: {kwargs}"
    )
    if doctype == "Pillar":
...
MOCK_SITE_PATH = os.path.join(
    tempfile.gettempdir(), "rcore_mock_bench", "sites", "test_site"
)
...
sys.modules["frappe.desk.doctype.workspace.workspace"] = MockModule(
...
def verify_onboarding_integration():
    ...
    try:
        ...
        assert len(bus_template) == 8, (
            "Business template must contain 8 standard questions."
        )
        assert len(life_template) == 8, (
            "Life template must contain 8 standard questions."
        )
    ...
    try:
        ...
        sample_milestones = [
            {
                ...
                "text": "Completed core dynamic dynamic execution API",
            },
            {
                "date": "2026-05-24",
                "category": "Product",
                "text": "Launched test harness simulator suite",
            },
        ]

        # Setup local test templates folder under StartupOS/templates dynamically
        monorepo_templates = os.path.join(
            parent_workspace_dir, "Monorepo", "templates_test"
        )
        ...
        if os.path.exists(monorepo_templates) and not os.path.exists(dest_templates):
            ...
            print(
                f"   [Test Setup] Copied templates from Monorepo to test directory: {dest_templates}"
            )
        ...
        questions_path = os.path.join(
            startup_os_root, "instances", "business", "AntigravityLabs", "questions.md"
        )
        output_dir = os.path.join(
            startup_os_root, "instances", "business", "AntigravityLabs", "output"
        )

        assert os.path.exists(questions_path), (
            "File questions.md must exist on filesystem."
        )

        if os.path.exists(output_dir):
            print(
                "✅ Filesystem side-effects (Real Compilation) verified successfully!"
            )
            print(f"   - questions.md size: {os.path.getsize(questions_path)} bytes")
            print(f"   - Compiled files generated: {os.listdir(output_dir)}")
        else:
            print(
                "✅ Filesystem side-effects (Stubbed/Mocked Compilation) verified successfully!"
            )
            print(f"   - questions.md size: {os.path.getsize(questions_path)} bytes")
            print(
                "   - Compiled files: skipped (stub compiler in standalone test mode)"
            )
...
"""
        ...
        assert "seeded from compiled strategic files" in db_res.get("message"), (
            "Seeder must use compiled files."
        )

        print(
            "✅ STEP 3 SUCCESS: Dynamic strategic markdown parser and seeder verified successfully."
        )
