...
class TestToDoLinking(FrappeTestCase):
    def setUp(self):
        ...
        if not frappe.db.exists("Vision", "Link Vision"):
             self.vision = frappe.get_doc({"doctype": "Vision", "title": "Link Vision"}).insert(ignore_permissions=True)
        else:
             self.vision = frappe.get_doc("Vision", "Link Vision")

        if not frappe.db.exists("Pillar", "Link Pillar"):
             self.pillar = frappe.get_doc({"doctype": "Pillar", "title": "Link Pillar", "vision": self.vision.name}).insert(ignore_permissions=True)
        else:
